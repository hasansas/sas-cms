...
class System{
	...
	function adminTheme(){
		...
		$activeTheme = 'sas';
		$adminTheme  = 'sas-admin/themes/'.$activeTheme.'/';
	...
	function load_js($jsPath, $asinyc=''){
		$load_js = '<script type="text/javascript" src="'.$jsPath.'?v.'.mt_rand(1,9).'.'.mt_rand(1,9).'" '.$asinyc.'></script>';
		echo $load_js;
	}
	...
	function thisPageID(){
		...
		if($this->thisPage()=='read'){
			...
			$qryPage 	= "select pa.page_id from sas_pages pa left join sas_posts po on(pa.category_id=po.post_category) where po.post_id='".$postsId."'";
	...
	function pageTitle(){
		...
		if($this->thisPage()=='read'){
			...
			$postsId 	= intval($this->uri(2));
			$qryPage 	= "select pa.page_name from sas_pages pa left join sas_posts po on(pa.category_id=po.post_category) where po.post_id='".$postsId."'";
			$pageTitle	= $this->db->getOne($qryPage);
		}
		elseif($this->thisModule()=='project' && intval($this->uri(2)>0)){

			$postsId 	= intval($this->uri(2));
			$qryPage 	= "select post_title from sas_posts where post_id='".$postsId."'";
			$pageTitle	= $this->db->getOne($qryPage);
		}
	...
	function generateDrawerMenu($arrMenu,$parent,$menuId,$title,$url,$customLinks,$adminMenu,$class,$showIcon=false){

		if(isset($arrMenu[$parent])){

			$dropdownMenu = '';
			$menu 	 = $parent==0?'<ul class="'.$class.'">':'<ul class="drawer-dropdown-menu">';
			$isAdmin = $this->uri(1)==$this->admin_name?1:0;
			...
			foreach($arrMenu[$parent] as $value){
				...
				$child  = $this->generateDrawerMenu($arrMenu,$value[$menuId],$menuId,$title,$url,$customLinks,$adminMenu,$class,$showIcon);

				if(!empty($value[$title])){

					if($child){
						$active = $value[$menuId]==$this->getParentID($this->thisMenuID())||$value[$menuId]==$mainParentID||$value[$menuId]==$thisMenuID?' active':'';
						$menu .= '<li class="drawer-dropdown'.@$active.'" id="m'.$value[$menuId].'" data-child="haschild"><a href="'.$seturl.'" data-toggle="dropdown" class="drawer-menu-item"'.$target.' role="button" aria-expanded="false">'.$icon.'<span class="menu-text"> '.$value[$title].' </span><span class="drawer-caret"></span></a>'.$child.'</li>';
					}
					else{
						$active = $value[$menuId]==$mainParentID||$value[$menuId]==$thisMenuID?' class="active"':'';
						$menu .= '<li'.@$active.' id="m'.$value[$menuId].'" data-child="nochild"><a href="'.$seturl.'"'.$target.' class="drawer-menu-item">'.$icon.'<span class="menu-text"> '.$value[$title].' </span></a></li>';
					}
				}
			}
	...
	function getMenu($position='',$class,$showIcon=false, $style='default'){
		...
		if(isset($data)){
      if($style=='drawer'){
  			$getMenu = $this->generateDrawerMenu($data,0,'menu_id','title','url','custom_links','admin_menu',$class,$showIcon);
      }
      else{
  			$getMenu = $this->generateMenu($data,0,'menu_id','title','url','custom_links','admin_menu',$class,$showIcon);
      }
			return $getMenu;
		}
		else return null;
	...
	function sendMail($sendmail){
		...
		if($this->config['useMailer']){
			...
			require_once(systemPath.'plugins/PHPMailer/PHPMailerAutoload.php');
			...
			$mail->SMTPSecure 	= $this->config['emailSecure'];
	...
	function dbDriver(){
		...
		if(!in_array($config['db_diver'],$drivers)){echo 'Unknown database driver.';exit;}
	}
	...
	function install(){
		...
		if(file_exists(systemPath.'install/install.php')){
			if(!empty($config['db_name'])){
				$installation_dir = systemPath.'install/';
				$this->delete_dir($installation_dir);
				if(file_exists(systemPath.'install/install.php')){
					echo 'Can not remove installation folder. please remove it manualy';
				}
				else{
					$uri = !empty($_SERVER['HTTPS']) && ('on' == $_SERVER['HTTPS'])?'https://':'http://';
					$this->redirect($uri.$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI]);
					exit();
				}
			}else{
				require_once(systemPath.'install/install.php');
			}
			exit();
		}
		else{
	...
	/* ---------- Migration ---------- */
	function create_migration($path, $str){

		$migration = fopen($path, "w");

        fwrite($migration, $str);
        fclose($migration);
	}
}
?>

...
class Post{
	...
	function comment($postID='', $type='post'){

		global $system;

		$query 		 = "select comment_from, comment_content, comment_date from ".$system->table_prefix."comments where comment_post=".$postID." and comment_type='".$type."' and publish='1' order by comment_id";
			        ...
			        <div class="media">
				        ...
				        <div class="media-body">
							<div class="comment-item">
								...
								<p>'.htmlspecialchars_decode(html_entity_decode($comment['comment_content'])).'</p>
							</div>
				        </div>
			        </div>
				</li>
		...
		$comment = '
			...
			<form id="frmAddComment" name="frmAddComment" action="'.modulePath.'posts/post.comment.php" method="post">
				...
				<button type="submit" class="btn btn-default btn-lg">
					<span class="label right">Kirim komentar</span>
					<span class="spinner comment-progress" style="display:none">
						<i class="bounce1"></i>
						<i class="bounce2"></i>
						<i class="bounce3"></i>
					</span>
				</button>
				<input type="hidden" name="postid" value="'.intval($postID).'">
				<input type="hidden" name="comment_type" value="'.$type.'">

	...
	if(file_exists($imageUrl)){
		...
		switch(strtolower($type)){
			...
			case 'jpg':
				...
				imagejpeg($virtual_image,$thumbPath,75);
			break;
...
function anti_injection($str){
	...
	if(!in_array($config['db_diver'],$drivers)){
...
// }
function strEncode($str){
	return anti_injection(htmlentities(str_replace('\\','\\\\',utf8_encode($str)),ENT_QUOTES));
}
...
function simpleShare($label=false){

	global $system;

	$redirectUrl = $system->thisUrl();
	$title		 = $system->pageTitle();
	$facebookLabel = $label?'<span>Facebook</span>':'';
	$twitterLabel = $label?'<span>Twitter</span>':'';
	$googleplusLabel = $label?'<span>Google+</span>':'';
	$linkedinLabel = $label?'<span>Linkedin</span>':'';
	$pinterestLabel = $label?'<span>Pinterest</span>':'';
	$mailLabel = $label?'<span>Email</span>':'';
	$simpleShare = '

		<a class="facebook" href="http://www.facebook.com/sharer.php?u='.$redirectUrl.'" target="_blank"><i class="socicon-facebook"></i>'.$facebookLabel.'</a>
		<a class="twitter" href="http://twitter.com/share?url='.$redirectUrl.'&text='.$title.'" target="_blank"><i class="socicon-twitter"></i>'.$twitterLabel.'</a>
		<a class="google-plus" href="https://plus.google.com/share?url='.$redirectUrl.'" target="_blank"><i class="socicon-googleplus"></i>'.$googleplusLabel.'</a>
		<a class="linkedin" href="http://www.linkedin.com/shareArticle?mini=true&url='.$redirectUrl.'" target="_blank"><i class="socicon-linkedin"></i>'.$linkedinLabel.'</a>
		<a class="pinterest" href="javascript:void((function()%7Bvar%20e=document.createElement(\'script\');e.setAttribute(\'type\',\'text/javascript\');e.setAttribute(\'charset\',\'UTF-8\');e.setAttribute(\'src\',\'http://assets.pinterest.com/js/pinmarklet.js?r=\'+Math.random()*99999999);document.body.appendChild(e)%7D)());"><i class="socicon-pinterest"></i>'.$pinterestLabel.'</a>
	';
...
function inputMap( $str_value = '', $str_name = '' ,$addClass='inputpesan tbless'){
	...
	$out	 = '';
	$out 	.= '<script type="text/javascript" src="http://maps.google.com/maps/api/js?key='.$system->config['googleKey'].'"></script>';
	$out 	.= '
		...
		<div class="input-map">
			<div class="input-group">
				...
				<span class="input-group-btn">
						<button class="view'.$name.' btn btn-primary btn-flat" type="button">Get Map</button>
				</span>
			</div>

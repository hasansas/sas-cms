...
class form{
	...
	function insert($post,$tableName){
		...
		if(!$config['demo']){
			...
			if($bfResponse['error']){
				$_SESSION['form_response'] = $this->alert('error',$bfResponse['alert']);
			}
			else{
				if($this->db->execute($out)){
					...
					$_SESSION['form_response'] = $this->alert('success',$this->getLang('insert_success'));
					unset($_POST);

					//$this->userLog($post,$out,$tableName);
					$uri = !empty($_SERVER['HTTPS']) && ('on' == $_SERVER['HTTPS'])?'https://':'http://';
					$system->redirect($uri.$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI]);
					exit;
				}
				else $_SESSION['form_response'] = empty($flds)?'':$this->alert('error',$this->getLang('insert_failed'));
			}
		}
		else $_SESSION['form_response'] = $this->alert('error',$this->getLang('disable_insert'));
	}
		...
		if(!$config['demo']){
			...
			if($bfResponse['error']){
				...
				if($this->db->execute($out)){
					...
					$_SESSION['form_response'] = $this->alert('success',$this->getLang('update_success'));
				...
				else $_SESSION['form_response'] = empty($flds)?'':$this->alert('error',$this->getLang('update_failed'));
			}
		}
		else $_SESSION['form_response'] = $this->alert('error',$this->getLang('disable_update'));
	}
	...
	function getForm($act,$sqltable,$arrInput,$formName='form',$submitValue='Submit',$finishBotton=false,$resetBotton=false,$extra=''){
		...
		switch($act){
			...
			case 'edit':
				...
				if(isset($_POST['save_'.$formName])){$this->update($_POST,$table['name'],$table['id'],$table['key']);clearTemp(tmpPath);}
				break;
		}
		...
		$form .= @$_SESSION['form_response'];unset($_SESSION['form_response']);
		$i 	= 0;
		$xi = 0;
		foreach($arrInput as $input){
			...
			switch($input['type']){
				...
				case 'password':
					...
					$value = isset($_POST['md5_'.$input['name']])?$_POST['md5_'.$input['name']]:$value;
					$form .= '<div class="'.$divClass.'">';
					$form .= $label;
				...
				case 'select':
					...
					$opt 		= '';
					$arrOption  = array();
					if($ck = @unserialize(html_entity_decode($value))){
						$arrVal = $ck;
					}
					else{
						$arrVal = array($value);
				...
				case 'icon':
					...
					$iconValue = !empty($value)?$value:'fa fa-circle-o';
					...
					$form  	  .= '
						...
						<div class="'.$divClass.'">
							<div class="select-icon">
								...
								<span class="btn-select btn-selectpage" data-toggle="modal" data-target="#modal-'.$input['id'].'">'.$input['label'].'</span>
				...
				case 'daterangepicker':
					$value 		= isset($_POST[@$input['name']])?$postValue:html_entity_decode($input['value']);
					$form .= '<div class="'.$divClass.'">';
					$form .= $label;
					$form .= '

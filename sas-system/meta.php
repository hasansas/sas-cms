...
switch($this->thisModule()){
    ...
    case 'book':
		...
		if(is_numeric($this->uri(2))){
		    $titlemulti		= $this->site->isMultiLang()?'title_'.$this->active_lang():'title';
			$descriptmulti	= $this->site->isMultiLang()?'description_'.$this->active_lang():'description';
			$contentmulti	= $this->site->isMultiLang()?'sinopsis_'.$this->active_lang():'sinopsis';
			$tagmulti		= $this->site->isMultiLang()?'tag_'.$this->active_lang():'tag';
			$metaPostsId 	= intval($this->uri(2));
			$metaQuery	 	= "
				...
				select
					...
					".$tagmulti." as post_tag,
					cover
				from ".$this->table_prefix."book
				where book_id='".$metaPostsId."' and publish='1'
			";
			//echo $metaQuery;
			$getMeta = $this->db->getRow($metaQuery);
			extract($getMeta);
			...
			$metaDescription = substr($metaDescription,0,150);
			$metaTag 		 = !empty($post_tag)?$post_tag:$this->site->keyword();
			$metaImage 		 = !empty($post_image)?$cover:'default.jpg';
			...
			$fbMeta		= array(
				...
				'og:title' 		 => $metaTitle,
				'og:description' => $metaDescription,
				'og:image' 		 => uploadURL.'modules/book/thumbs/small/'.$metaImage
			);
		}
		else{
			$meta = array();
